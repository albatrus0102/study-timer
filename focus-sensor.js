...
function calFit(s, o) {
  ...
  if (fit < 0.75 || fit > 1.35) {
    ...
    // 상한에 걸리면 보정이 덜 된다 = 눈을 실제보다 더 뜬 것으로 본다.
    // 즉 졸음을 놓치는 쪽으로 기울지, 없는 졸음을 만들어내지는 않는다.
    // 불안하게 읽히지 않도록 이 방향을 분명히 밝힌다.
    warn += `<div class="warn">보정이 상한에 걸린 샘플이 ${(clampRate*100).toFixed(0)}% 입니다. ` +
      `캘리브레이션 자세(faceH ${s.cal.faceH.toFixed(2)})보다 고개를 든 시간이 많았다는 뜻입니다.<br>` +
      `<b>이 어긋남은 졸음을 “덜” 잡는 쪽으로만 작용합니다.</b> 없는 졸음을 만들어내지는 않으니 ` +
      `지금 리포트를 그대로 믿으셔도 됩니다. 더 정확하게 하고 싶으면 <b>평소 자세의 중간쯤</b>에서 ` +
      `다시 재세요 — 종이 문제풀이와 화면 강의처럼 자세가 크게 다른 두 상황을 오간다면 ` +
      `그 중간이 양쪽을 다 담습니다.</div>`;
  }
